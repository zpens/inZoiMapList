exports.handler = async (event) => {
  ...
  try {
    ...
    // Commit updated positions
    const content = Buffer.from(JSON.stringify(positions, null, 2)).toString('base64');
    const body = {
      message: `Update positions (${new Date().toISOString().slice(0, 16)})`,
      content,
      branch: BRANCH,
    };
    if (sha) body.sha = sha;
    ...
    return {
      ...
      body: JSON.stringify({ success: true, message: 'Positions saved and deploy triggered' }),
    };
  } catch (err) {
